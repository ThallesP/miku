...
**Restate**-powered workers that join a mesh network and run container deployments — all wired
...
| Package | Purpose |
| --- | --- |
| `packages/backend` | The control plane: a self-hosted [Convex](https://convex.dev) deployment (`convex/schema.ts` + `convex/{apps,workers,deployments}.ts`). One reactive datastore + typed functions; the generated `convex/_generated` is the shared contract imported by web and workers. |
...
| `apps/workers` | Worker process. Joins the network (Tailscale, with a local fallback), self-registers with Convex, heartbeats, subscribes to its deployments, and runs each via a local [Restate](https://restate.dev) durable workflow (`docker pull` → `docker run`). |
| `packages/network` | `joinNetwork` — `tailscale up` with a LAN fallback. |

## Architecture

```
UI ──useMutation──▶ Convex (apps / workers / deployments)        the brain: reactive state
                          │  live subscription (pull)
                          ▼
   worker ──onUpdate──▶ reconcile ──▶ local Restate (localhost)  the durable hands
                                            │ docker pull/run
                                            ▼
   worker ──updateStatus──▶ Convex  ──reactive──▶ UI updates live
```

The control plane is **Convex**, which can't run the `tailscale` CLI or reach into your tailnet —
so the model is inverted: **workers find the control plane and call out to it**. A worker boots,
joins the tailnet, and calls the public `workers.register` mutation (no auth in v1). It then
...
bun install
bun run dev                          # convex dev + web on :3000 + a worker
```

The dashboard is on `http://localhost:3000`, the Convex dashboard on `http://localhost:6791`.
Add an application, drag it (synced live across tabs), then deploy it to a worker — the worker
...
Worker env (all optional; defaults shown): `CONVEX_URL=http://localhost:3210`,
